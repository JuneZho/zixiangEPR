...
class MineBOManager(models.Manager):
    ...
    def get_query_set(self):
        return super(MineBOManager,self).get_query_set().filter(creator=cusor.getuser())
...
class BO(models.Model):
    ...
    end = models.DateField(_('结束日期'),blank=True,null=True)
    modifier = models.CharField(_("修改者"),blank=True,null=True,max_length=const.DB_CHAR_NAME_20)
